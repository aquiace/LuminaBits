...
#define LED_PIN   17  //Pin for the pixel strand. Can be analog or digital.
#define LED_TOTAL 125  //Change this to the number of LEDs in your strand.
...
#define KNOB_PIN  1  //Pin for the trimpot 10K
#define BUTTON_1  10   //Button 1 cycles color palettes
#define BUTTON_2  11   //Button 2 cycles visualization modes
#define BUTTON_3  12   //Button 3 toggles shuffle mode (automated changing of color and visual)

#define LASER_PIN 13
#define OTHER_LASER_PIN A2
int laserValue = 0;

int PowerMode = 0;

#define SPLINE_START 8
...
Adafruit_NeoPixel strand = Adafruit_NeoPixel(LED_TOTAL, LED_PIN, NEO_GRBW + NEO_KHZ800);   //LED strand objetcs
...
void ShootDalaser(int value)
{
  //digitalWrite(LASER_PIN, HIGH);
  //analogWrite(LASER_PIN, value);
  analogWrite(OTHER_LASER_PIN, value);
}

void OffDaLaser()
{
  //digitalWrite(LASER_PIN, LOW);
  digitalWrite(OTHER_LASER_PIN, LOW);
}
...
void TogglePower()
{
  if(PowerMode == 1)
  {
    Serial.println("Power Mode: " + PowerMode);
     for(int i = 0; i < LED_TOTAL; i++)
     {
        Serial.print(i);
        strand.setPixelColor(i, 0);
     }
     strand.show();
     PowerMode = 0;
  }
...
void Pulse() {
  ...
  if (volume > 0) {
    ...
    int start = wingsFloor(LED_HALF - (LED_HALF * (volume / maxVol)));
...
void Glitter() {
  ...
  if (bump) {
    ...
    dotPos = wingsFloor(random(strand.numPixels() - 1));
...
void Wings()
{
  ...
  for(int i = 0; i < 8; i++){
    //strand.setPixelColor(i, strand.Color(col, col, col));
    strand.setPixelColor(i, 155, 155, 255, 200);
  }
  //strand.show();
}
